...
for k=1:n_rep
    ...
    %             isConstrained = isConstrained || (constrains(i).min_q1 <= D(k,m).w(1,n) && D(k,m).w(1,n) <= constrains(i).max_q1 && constrains(i).min_q2 <=D(k,m).w(2,n) && D(k,m).w(2,n)<=constrains(i).max_q2);
                isConstrained = isConstrained || (constrains(i).min_w1-0.01 <= D(k,m).w(1,n) && D(k,m).w(1,n) <= constrains(i).max_w1+0.01 && constrains(i).min_w2-0.01 <=D(k,m).w(2,n) && D(k,m).w(2,n)<=constrains(i).max_w2+0.01 );
            end
            if isConstrained
