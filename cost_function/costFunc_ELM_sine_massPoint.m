            ...
            isConstrained = 0;
                for i = 1:length(constraints)
                    ...
                    elseif strcmp(constraints(i).type, 'circle')
                        isConstrained = isConstrained || norm(D(k,m).w(1,n)-[constraints(i).center1,constraints(i).center2] ) < constraints(i).radius + 0.6 ;
                    end
                end
                
                
            if isConstrained
                viapointCostSteps(n,k,m) = viapointCostSteps(n,k,m)+ 0.005;
            end
        end
...
accelerationCostWeight = 0.001;
transCostWeight = 0.003;
% viapointCostSteps
% viapointCostSteps
