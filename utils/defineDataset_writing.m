function trainingData = defineDataset_2Dletter(demos,p)
%AUGMENTDATA 
...
%     sample(:,:,n) = spline(1:size(demos{n},2), demos{n}(1:n_dim,:), linspace(1,size(demos{n},2),n_steps)); %Resampling
    sample(:,:,n) = spline(1:size(demos{n}.pos,2), demos{n}.pos(1:n_dim,:), linspace(1,size(demos{n}.pos,2),n_steps)); %Resampling
end                    
